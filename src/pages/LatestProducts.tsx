...
import { ChevronLeft, Trophy, Sparkles, Star } from "lucide-react";
...
export default function LatestProducts() {
  ...
  const latestProducts = useMemo(() => {
    ...
    // Sort generically, or by id descending assuming higher ID = newer
        ...
        <section>
          {isLoadingProducts ? (
            <div className="flex justify-center py-12">
              <DotsLoader className="text-[#111827]" />
            </div>
              ...
              {latestProducts.map((product) => (
                ...
                >
                  <div className="aspect-[4/5] overflow-hidden relative bg-gray-50 p-3">
                    <span className="absolute top-2 left-2 bg-[var(--color-primary)] text-white text-[10px] font-black px-2 py-1 rounded-sm z-10 shadow-sm hidden group-hover:block">
                      جديد
                    </span>
                    <img
